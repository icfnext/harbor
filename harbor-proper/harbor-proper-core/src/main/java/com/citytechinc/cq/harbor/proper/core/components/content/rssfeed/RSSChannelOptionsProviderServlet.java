...
@SlingServlet(
    resourceTypes = RSSFeed.RESOURCE_TYPE,
    selectors = "options",
    methods = "GET",
    extensions = "json"
)
public class RSSChannelOptionsProviderServlet extends AbstractOptionsProviderServlet {
    ...
    @Reference(
            ...
            bind = "bindResourceProvider",
            unbind = "unbindResourceProvider",
    ...
    protected List<Option> getOptions(ComponentServletRequest componentServletRequest) {
        ...
        for (RssResourceProvider currentRssResourceProvider : localResourceProviders) {
            optionList.add(new Option(currentRssResourceProvider.getProviderRoot(), currentRssResourceProvider.getTitle()));
        }
