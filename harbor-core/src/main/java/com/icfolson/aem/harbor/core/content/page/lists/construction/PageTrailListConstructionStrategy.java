...
public class PageTrailListConstructionStrategy extends AbstractComponent implements ListConstructionStrategy<TrailPage> {
	...
	protected Predicate<PageDecorator> getRootPagePredicate() {
		if (rootPagePredicate == null) {
			String rootPageType = this.isInherits() ? getInherited("rootPageType", HomePage.RDF_TYPE) : get("rootPageType", HomePage.RDF_TYPE);
	...
	protected Boolean isIncludeCurrentPageInTrail() {
		if (includeCurrentPageInTrail == null) {
			includeCurrentPageInTrail = this.isInherits() ? getInherited("includeCurrentPageInTrail", false) : get("includeCurrentPageInTrail", false);
		}
	...
	protected Boolean isIncludeRootPageInTrail() {
		if (includeRootPageInTrail == null) {
			includeRootPageInTrail = this.isInherits() ? getInherited("includeRootPageInTrail", false) : get("includeRootPageInTrail", false);
		}
