package com.citytechinc.aem.harbor.core.services.sitemap.impl;
...
import static com.citytechinc.aem.namespace.api.ontology.Properties.CITYTECH_SITEMAP_RESOURCE_EXTENSION;
...
import com.citytechinc.aem.harbor.core.domain.sitemap.ChangeFrequency;
...
public class DefaultSiteMapService implements SiteMapService {
	...
	protected String determineChangeFrequency(final ValueMap contentResourceValueMap) {
		final String specifiedChangeFrequency = contentResourceValueMap.get(CITYTECH_SITEMAP_CHANGE_FREQUENCY, null);
		...
		if (isEmpty(specifiedChangeFrequency)) {
			LOG.warn(logTemplateEmptyFrequency,
				new Object[] { CITYTECH_SITEMAP_CHANGE_FREQUENCY, ChangeFrequency.valuesString() });
			return null;
	...
	protected String determinePriority(final ValueMap contentResourceValueMap) {
		final String specifiedPriority = contentResourceValueMap.get(CITYTECH_SITEMAP_PRIORITY, null);
		...
		if (null == parsedSpecifiedPriority
			...
			LOG.error(logTemplateInvalidPriority, CITYTECH_SITEMAP_PRIORITY);
			return null;
		} else {
	...
	protected String determineLoc(final ResourceResolver resourceResolver, final PageDecorator pageDecorator,
		...
		final String extension = contentResourceValueMap.get(CITYTECH_SITEMAP_RESOURCE_EXTENSION, EMPTY);
