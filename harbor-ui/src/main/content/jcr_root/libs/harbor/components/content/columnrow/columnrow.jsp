...
<div class="${ColumnRow.cssClass}" <c:if test="${ColumnRow.hasDomId}">id="${ColumnRow.domId}"</c:if>>
    <c:forEach var="curcolumn" items="${curRow.columns}" varStatus="status">
        <c:choose>
            <c:when test="${curcolumn.classification.hasClassifications}">
                <div class="${curRow.gridSize}${curcolumn.colSize} ${curcolumn.classification.classNames}" name="${curcolumn.name}" <c:if test="${curcolumn.hasDomId}">id="${curcolumn.domId}"</c:if>>
