...
<c:choose>
    <c:when test="${contentContainer.section}">
        <section <c:if test="${contentContainer.classification.hasClassification}">class="${contentContainer.sectionClass}"</c:if> <c:if test="${contentContainer.hasDomId}">id="${contentContainer.domId}"</c:if>>
